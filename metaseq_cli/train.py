...
def main(cfg: DictConfig) -> None:
    ...
    # logger.info(f"{task}, {task.build_model}")
    # return
    logger.info("start constructing the model")
    model = task.build_model(cfg.model)
    model = model.to(torch.float16)
    # torch.save(model, "/data/home/linjianma/metaseq/results/save.pt")
    # return
    ...
    # max_epoch = 5 # cfg.optimization.max_epoch or math.inf
    train_meter = meters.StopwatchMeter()
    train_meter.start()
    if True: #epoch_itr.next_epoch_idx <= max_epoch:
        # train for one epoch
        # valid_losses, should_stop =
...
def train(
    ...
    def train(
        ...
        # )
        valid_losses, should_stop = 1.11111, False
